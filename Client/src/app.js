...
app.get('/pedido/delete/:id', async (req, res) => {

    let result = await fetch(API_BASE_URL + req.params.id, {
        method: 'DELETE'
    })
...
app.get('/pedido/:id/qrcode', (req,res) =>{
    fetch(API_BASE_URL + `${req.params.id}/boleto`)
    .then(data => data.json())
    .then(result => {
        res.render('qrcode',{base64String:result.data.pix.qrcode_image});
    })
    .catch(err=>{
        res.sendStatus(500);
    });
            ...
            fetch(result.data.pdf.charge)
                ...
                .then(arrayBuffer => {
                    ...
                    return new Promise((resolve, reject) => {
                        fs.writeFile( path.join(__dirname, 'bankSlips', `file${dateNow}.pdf`), buffer, (err) => {
                            if (err) {
                                reject()
                            } else {
                                resolve(dateNow);
                    ...
                    let filePath = path.join(__dirname, 'bankSlips', `file${fileDate}.pdf`);
